...
namespace RUMTree
{
    ...
    public class RUMTree<T>
    {
        ...
        public Dictionary<int, T> IdsToItems = new Dictionary<int, T>();
        public Dictionary<T, int> ItemsToIds = new Dictionary<T, int>();
        ...
        //UpdateMemo and List
        private StampGenerator stampCnter = new StampGenerator();

        private LinkedList<Node<T>> nodeList = new LinkedList<Node<T>>();
        private GarbageCollectCtrl<T> gc;
        public Dictionary<int, UpdatedMemo> OIDsToUM=new Dictionary<int, UpdatedMemo>();
        /// <summary>
        ...
        ///</param>
        public RUMTree(int MaxNodeEntries, int MinNodeEntries)
        {
        ...
        {
            //initialize logs
            log = LogManager.GetLogger(typeof(RUMTree<T>).FullName);
            deleteLog = LogManager.GetLogger(typeof(RUMTree<T>).FullName + "-delete");
            ...
            Node<T> root = new Node<T>(rootNodeId, 1, maxNodeEntries, OIDsToUM, IdsToItems);
            nodeMap.Add(rootNodeId, root);
            
            log.Info("init() " + " MaxNodeEntries = " + maxNodeEntries + ", MinNodeEntries = " + minNodeEntries);
            //RUM tree
            this.gc = new GarbageCollectCtrl<T>(nodeList, this);
        ...
        public void Add(Rectangle r, T item)
        {
            ...
            BigInteger stamp = stampCnter.getStamp();
            add(r, id, stamp);
           
        }

        private void add(Rectangle r, int id, BigInteger stamp)
        {
        ...
        private void add(Rectangle r, int id, int level, BigInteger stamp)
        {
            ...
            {
                n.addEntryNoCopy(r, id, stamp);
            }
            else
            {
                newLeaf = splitNode(n, r, id,stamp);
                //RUMTree
               //if (newLeaf.isLeaf())
            ...
            //RUMTree
          //  if (newLeaf != null) nodeList.AddLast(newLeaf);
           // if (newNode != newLeaf && newNode!=null) nodeList.AddLast(newNode);
            //end
            ...
            if (newNode != null)
            {
                ...
                treeHeight++;
                Node<T> root = new Node<T>(rootNodeId, treeHeight, maxNodeEntries, OIDsToUM, IdsToItems);
                root.addEntry(newNode.mbr, newNode.nodeId, stamp);
                root.addEntry(oldRoot.mbr, oldRoot.nodeId, stamp);
                nodeMap.Add(rootNodeId, root);
                //RUM tree
                nodeList.AddLast(root);
                //end
        ...
        public bool Delete(Rectangle r, T item)
        {
            ...
            return delBasedOnUM(item);
        }

        private bool delBasedOnUM( T item)
        {
            int oid = ItemsToIds[item];
            BigInteger stamp = stampCnter.getStamp();
            if (OIDsToUM.ContainsKey(oid))
            {
                OIDsToUM[oid].incrementObsoleteEntryCnt();
                OIDsToUM[oid].setLatestStamp(stamp);
            }
            else
            {
                OIDsToUM.Add(oid, new UpdatedMemo(oid, 1, stamp));
            }

       
                IdsToItems.Remove(oid);
                ItemsToIds.Remove(item);
            
            gc.update();
            return true;
        }
        public int gcCalled = 0;
        public void garbageCollect(Node<T> targetNode)
        {
            gcCalled++;
           ...
           /* // shrink the tree if possible (i.e. if root Node&lt;T%gt; has exactly one entry,and that 
            ...
            if (foundIndex != -1)
            {
                int removedItemCnt = n.collectGarbage(minNodeEntries);
              //  n.deleteEntry(foundIndex, minNodeEntries);
                condenseTree(n);
                msize -= removedItemCnt;
            }
        ...
        */
        ...
        private Node<T> splitNode(Node<T> n, Rectangle newRect, int newId, BigInteger stamp)
        {
            ...
            Node<T> newNode = null;
            newNode = new Node<T>(getNextNodeId(), n.level, maxNodeEntries, OIDsToUM, IdsToItems);
            nodeMap.Add(newNode.nodeId, newNode);
            //RUM Tree
            nodeList.AddLast(newNode); 
            //end
            pickSeeds(n, newRect, newId, newNode, stamp); // this also sets the entryCount to 1
            ...
            while (n.entryCount + newNode.entryCount < maxNodeEntries + 1)
            {
                ...
                if (maxNodeEntries + 1 - n.entryCount == minNodeEntries)
                {
                    ...
                    for (int i = 0; i < maxNodeEntries; i++)
                    {
                        ...
                        {
                            entryStatus[i] = ENTRY_STATUS_ASSIGNED;
                            newNode.addEntryNoCopy(n.entries[i], n.ids[i],n.stamps[i]);
                            n.entries[i] = null;
                        }
                    }
                    break;
            ...
            if (log.IsDebugEnabled)
            {
                double newArea = n.mbr.area() + newNode.mbr.area();
        ...
        private void pickSeeds(Node<T> n, Rectangle newRect, int newId, Node<T> newNode, BigInteger stamp)
        {
            ...
            {
                newNode.addEntry(newRect, newId, stamp);
            }
            else
            {
                newNode.addEntryNoCopy(n.entries[highestLowIndex], n.ids[highestLowIndex], n.stamps[highestLowIndex]);
                ...
                n.stamps[highestLowIndex] = stamp;
            }
        ...
        {
            double maxDifference = float.NegativeInfinity;
            int next = 0;
            int nextGroup = 0;
            ...
            {
                // move to new node.
                newNode.addEntryNoCopy(n.entries[next], n.ids[next],n.stamps[next]);
                n.entries[next] = null;
            }
        ...
        private Node<T> adjustTree(Node<T> n, Node<T> nn)
        {
            ...
            while (n.level != treeHeight)
            {
                ...
                if (nn != null)
                {
                    ...
                    {
                        parent.addEntry(nn.mbr, nn.nodeId, -1); //RUM tree- no stamp, no obsolete entry attached.
                    }
                    else
                    {
                        newNode = splitNode(parent, nn.mbr.copy(), nn.nodeId, -1);//RUM tree- no stamp, no obsolete entry attached.
                    }
                }
